...
export default function ReportView({ report }: { report: Report }) {
  ...
  const recommended = report.paths.find((p) => p.recommended) ?? report.paths[0];
  const scoreColor =
      ...
      <div ref={reportRef} className="bg-white">
        ...
        <header className="mb-8">
          <h1 className="text-3xl font-bold">
            Health Score: <span className={scoreColor}>{report.healthScore}/100</span> ({report.healthLabel})
          </h1>
          <p className="mt-2 text-gray-700">{report.primaryConcern}</p>
        </header>

        <section className="grid gap-4 rounded-xl bg-gray-50 p-6 md:grid-cols-3">
          <Stat label="Total debt" value={lakh(report.totalDebt)} />
          <Stat label="Monthly outflow" value={lakh(report.monthlyOutflow)} />
          <Stat label="Debt-to-income" value={`${report.debtToIncomePercent.toFixed(0)}%`} />
        </section>
                ...
                {report.paths.map((p) => (
                  <tr key={p.pathId} className="border-b align-top">
                    <td className="p-3">
                      ...
                      {p.warning && <p className="mt-1 text-xs text-red-600">⚠ {p.warning}</p>}
                    </td>
